...
class BaseOptimizer(ABC):
    ...
    def optimize(self, problem=None, X=None, y=None, objective_function=None):
        ...
        def optimize(self, problem=None, X=None, y=None, objective_function=None):
            ...
            start_time = time.time()
            self._initialize_parameters(X, y, objective_function)
            optimizer_generator = self._optimize(
        objective_function=objective_function, X=X, y=y)
    
            final_iteration_data = None
            for iteration_data in optimizer_generator:
                # This loop will run until the generator is exhausted (or stopped by a wrapper).
                # We only care about the very last thing it yielded.
                final_iteration_data = iteration_data

            if final_iteration_data is None:
                raise RuntimeError(f"The '{self.algorithm_name_}' optimizer did not yield any results. Please check its implementation.")

            # Unpack the final result from the last yielded dictionary
            best_solution, best_fitness, global_fitness, local_fitness, local_positions = final_iteration_data['final_result']
            end_time = time.time()
            execution_time = end_time - start_time
            model = self._create_model(
                best_solution, best_fitness, global_fitness, execution_time, X, y, local_fitness, local_positions)
