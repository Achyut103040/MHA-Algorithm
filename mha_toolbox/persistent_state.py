...
# Import streamlit-cookies-manager for browser-based session persistence
try:
    from streamlit_cookies_manager import EncryptedCookieManager
    COOKIES_AVAILABLE = True
except ImportError:
    COOKIES_AVAILABLE = False
    print("Warning: streamlit-cookies-manager not available. Browser session persistence disabled.")
...
class PersistentStateManager:
    ...
    def __init__(self, base_dir="persistent_state"):
        ...
        # Initialize cookie manager for browser-based persistence
        if COOKIES_AVAILABLE:
            self.cookies = EncryptedCookieManager(
                prefix="mha_toolbox_",
                password="mha_secure_key_2024"  # In production, use environment variable
            )
    ...
    def get_session_id_from_browser(self):
        """
        Retrieve session ID from browser cookies.
        Returns session_id if found, None otherwise.
        
        This enables "Welcome Back" functionality.
        """
        if not COOKIES_AVAILABLE or self.cookies is None:
            return None
        
        try:
            # Read cookie named 'mha_session_id'
            session_id = self.cookies.get('mha_session_id')
            
            if session_id:
                # Validate session exists
                session_dir = Path("results/detailed_storage") / session_id
                if session_dir.exists():
                    return session_id
                else:
                    # Cookie points to non-existent session, clear it
                    self.cookies['mha_session_id'] = ''
                    self.cookies.save()
                    return None
            ...
            st.warning(f"Could not read session cookie: {e}")
            return None
    
    def set_session_id_in_browser(self, session_id):
        """
        Store session ID in browser cookies for persistent recognition.
        ...
        try:
            # Set cookie named 'mha_session_id'
            self.cookies['mha_session_id'] = session_id
            self.cookies.save()
            return True
